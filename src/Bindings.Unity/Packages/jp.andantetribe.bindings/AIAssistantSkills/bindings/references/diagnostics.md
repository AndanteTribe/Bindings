...
| ID | レベル | 条件 | 修正 | 生成動作 |
| --- | --- | --- | --- | --- |
| `BND001` | Error | `[ViewModel]` 型名に `ViewModel` が含まれない。 | 型名を `SomethingViewModel` へ変更し、参照も更新する。 | ViewModel の補助ソースは生成するが、View は生成しない。 |
...
| `BND005` | Warning | `[ViewModel]` 型のフィールドを Unity の `[SerializeField]` でシリアライズしている。 | ViewModel をコードまたは DI で構築する。参照シリアライズが意図的な場合だけ `[SerializeReference]` を検討する。 | 対象 ViewModel と View の生成を継続する。 |
