...
生成されるバインディングでは `_isVisibleGameObject.SetActive(_viewModel.IsVisible);`（または相当する生成ヘルパー）が呼ばれ、アクティブ状態が反映されます。
...
| ID | レベル | 条件 |
|----|--------|------|
| `BND001` | エラー | `[ViewModel]` を付与したクラス名に `"ViewModel"` が含まれていない。ViewModel と View のどちらも生成しない。 |
